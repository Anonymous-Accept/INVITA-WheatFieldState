"""Run the Sensor-sequence TCN neural representation."""
...
from src.benchmarks.baselines.sensor_sequence_transformer import SensorSequenceDataLoader  # noqa: E402
...
from src.benchmarks.neural_baselines.common.torch_tcn import (  # noqa: E402
    SensorSequenceTCNConfig,
    SensorSequenceTCNRegressor,
)
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--output-dir",
        type=Path,
        default=default_output_root() / "neural_baselines" / "sensor_sequence_tcn",
    )
...
def run(args: argparse.Namespace) -> Path:
    ...
    model_config = SensorSequenceTCNConfig(
    ...
    write_json(
        {
            "experiment_id": "sensor_sequence_tcn",
            "paper_method": "Sensor-sequence TCN",
            "run_id": args.run_id,
            ...
            "allowed_inputs": ["Sensor-summary model/Sensor-sequence Transformer legal pre-target GreenSeeker and UAV-MS history bins"],
            "disallowed_inputs": ["field-camera images", "raster values", "post-target observations", "target manifests as features"],
    ...
    write_readme(
        ...
        title="Sensor-sequence TCN",
        body=(
            "Temporal convolutional neural representation over the same legal pre-target "
            "sensor-history sequence bins used by Sensor-sequence Transformer. Rows without required "
            "sensor history are excluded by the reused Sensor-sequence Transformer/Sensor-summary model dataloader."
        ),
    )
    ...
    for task_name in args.tasks:
        ...
        coverage_rows.append(_coverage_row(task_name, task_data))
        if task_data.train.empty:
            coverage_rows[-1]["status"] = "blocked_empty_train"
            continue
        model = SensorSequenceTCNRegressor(model_config).fit(task_data.train, task_data.val)
        write_json(model.training_summary, run_dir / task_name / "training_summary.json")
        training_rows.append(
            training_summary_row(
                task_name=task_name,
                baseline_name="sensor_sequence_tcn",
                model=model,
            ...
            torch.save(
                {
                    ...
                    "value_std": model.value_std,
                    "count_mean": model.count_mean,
                    "count_std": model.count_std,
    ...
    logger.info("Sensor-sequence TCN run complete: %s", run_dir)
    return run_dir
...
def _coverage_row(task_name: str, task_data: BaselineTaskData) -> dict:
    return {
        "task": task_name,
        "method": "Sensor-sequence TCN",
        "required_observations": "legal pre-target sensor history",
        "train": len(task_data.train),
        "val": len(task_data.val),
        "test": len(task_data.test),
        "coverage_percent": None,
        "status": "ran" if len(task_data.test) else "no_test_rows_after_sensor_history_filter",
    }
