"""Run the Observation-availability model experiment."""
...
from src.benchmarks.baselines.observation_availability_model import (  # noqa: E402
    ObservationAvailabilityModel,
    ObservationAvailabilityConfig,
    ObservationAvailabilityDataLoader,
)
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--output-dir",
        type=Path,
        default=default_output_root() / "observation_availability_model",
    )
...
def run(args: argparse.Namespace) -> Path:
    ...
    model_config = ObservationAvailabilityConfig(
    ...
    run_config = {
        "baseline": "Observation-availability model",
        "run_id": run_id,
        ...
        "temporal_policy": "Dynamic input assets are counted only when acquisition_date < target_date. The current target source asset is excluded.",
        "created_utc": datetime.now(timezone.utc).isoformat(),
        "allowed_inputs": ["inputs_index modality names", "input counts", "pre-target input timing", "target_name"],
        "disallowed_inputs": [
            "payload values",
            "sensor values",
            "raster values",
            "field-camera image pixels or embeddings",
            "weather values",
            "post-target input counts",
            "same-source target asset",
        ],
    }
    write_json(run_config, run_dir / "run_config.json")

    loader = ObservationAvailabilityDataLoader(
    ...
    for task_name in args.tasks:
        ...
        model = ObservationAvailabilityModel(model_config)
        model.fit(task_data.train)
        task_metrics, task_target_metrics, task_fallback = evaluate_task_model(
            model=model,
            task_data=task_data,
            baseline_name="Observation-availability model",
            eval_splits=args.eval_splits,
    ...
    logger.info("Observation-availability model run complete: %s", run_dir)
    return run_dir
