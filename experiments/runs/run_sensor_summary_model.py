"""Run the Sensor-summary model safe sensor-history experiment."""
...
from src.benchmarks.baselines.sensor_summary_model import (  # noqa: E402
    SensorSummaryConfig,
    SensorSummaryDataLoader,
    SensorSummaryModel,
)
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--output-dir", type=Path, default=default_output_root() / "sensor_summary_model"
    )
...
def run(args: argparse.Namespace) -> Path:
    ...
    model_config = SensorSummaryConfig(
    ...
    write_json(
        {
            ...
            "temporal_policy": "Only observations strictly before target_date are summarized; the current source asset is excluded.",
            "implementation_assumption": "Uses true payload CSV values from legal pre-target GreenSeeker and UAV-MS inputs_index assets; target manifests are not used as feature values.",
            "required_modality_policy": "Targets without at least one legal pre-target payload-backed sensor observation are excluded from Sensor-summary model.",
            "created_utc": datetime.now(timezone.utc).isoformat(),
        },
        run_dir / "run_config.json",
    )

    loader = SensorSummaryDataLoader(
    ...
    for task_name in args.tasks:
        ...
        if task_data.train.empty:
            logger.warning(
                "%s skipped: no Sensor-summary model training rows after modality filtering", task_name
            )
            continue
        model = SensorSummaryModel(model_config).fit(task_data.train)
        task_metrics, task_target_metrics, task_fallback = evaluate_task_model(
            model=model,
            task_data=task_data,
            baseline_name="Sensor-summary model",
            eval_splits=args.eval_splits,
    ...
    logger.info("Sensor-summary model run complete: %s", run_dir)
    return run_dir
