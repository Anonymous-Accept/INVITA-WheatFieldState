"""Sensor-summary model safe sensor-history model baseline."""
...
@dataclass
class SensorSummaryConfig:
    """Configuration for Sensor-summary model sensor-history model."""
...
class SensorSummaryModel:
    """Gradient boosting model over pre-target sensor-history summaries."""

    def __init__(self, config: SensorSummaryConfig | None = None) -> None:
        self.config = config or SensorSummaryConfig()
        self.model = None
    ...
    def fit(self, data: pd.DataFrame) -> SensorSummaryModel:
    ...
    def _prepare_frame(
        ...
        for col in self.config.categorical_features:
            if col not in frame.columns:
                raise ValueError(f"Missing required Sensor-summary model categorical column: {col}")
            frame[col] = normalize_categorical(frame[col])

        if fit:
            self._numeric_features = self.config.numeric_features or sorted(
                col for col in frame.columns if col.startswith("hist_")
            )
        missing = set(self._numeric_features) - set(frame.columns)
        if missing:
            raise ValueError(f"Missing required Sensor-summary model numeric columns: {sorted(missing)}")
